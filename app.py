...
st.markdown("## 🟣【AI評価プロンプト（必ずコピペ→AIに貼り付け！）】")
st.code(ai_prompt, language="markdown")

st.markdown("""
### 🟢【AI評価・画像アップロード前の流れ】

1. **下記プロンプトをAIチャット（ChatGPT/Claudeなど）に“必ずコピペ”して貼り付ける。**
2. **AIが『画像をアップロードしてください』と返すのを“必ず確認”する。**
3. **その後、画像10枚までをAIに一括アップロードする。**
4. **AIがCSV形式で全画像分の評価を返すので、そのまま下記の「評価済みCSVアップロード」でシステムに流し込む。**
5. **スコア＋コメント付き画像をZIPで一括DL。**

> ※この順番を守れば“どうしますか”確認や分岐・事故ゼロ。  
> （プロンプトをAIに貼る→AIが「画像をください」と返す→画像を渡す→CSV返答→アップロード）
---
""")
